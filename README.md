...
2. Print/echo $this->layout->view at the point in your layout which can change depending on the page you are viewing
...
7. Create different layouts if you need to and in the views which you want to use those layouts begin with <?php $this->setLayoutName('layout2'); ?> (notice the missing .php extension)
8. Create view helpers in /views/helpers (see Button.php as an example). These are simply classes which extend ViewHelper in /lib/ViewHelper.php and implement function invoke(). invoke() can accept any number of arguments and can be called from with a layout or view by simply calling <?php echo $this->viewHelperName([$arg1, [$arg2, ...]]); ?> - eg. <?php echo $this->button('Button text'); ?>
